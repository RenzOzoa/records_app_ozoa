...
if (strlen($search) > 0) {
    ...
    $query = "SELECT transaction.datelog, transaction.documentcode, transaction.action, office.name as office_name, CONCAT(employee.lastname,',', employee.firstname) as employee_fullname, transaction.remarks 
              FROM recordsapp.employee, recordsapp.office, recordsapp.transaction 
              WHERE transaction.employee_id=employee.id and transaction.office_id = office.id and transaction.documentcode = '$search' ORDER BY CAST(transaction.documentcode AS SIGNED), transaction.datelog LIMIT $page_first_result, $result_per_page";
} else {
    $query = "SELECT transaction.datelog, transaction.documentcode, transaction.action, office.name as office_name, CONCAT(employee.lastname,',', employee.firstname) as employee_fullname, transaction.remarks 
              FROM recordsapp.employee, recordsapp.office, recordsapp.transaction 
              WHERE transaction.employee_id=employee.id and transaction.office_id = office.id ORDER BY CAST(transaction.documentcode AS SIGNED), transaction.datelog LIMIT $page_first_result, $result_per_page";
}
